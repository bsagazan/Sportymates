...
switch($action)
{
    ...
    if (empty($message) || empty($titre))
    {
        ...
        cliquez <a href="../vue/forum/poster.php?action=nouveautopic&amp;f='.$forum.'">ici</a> pour recommencer</p>';
    }
    else //Si jamais le message n'est pas vide
    {
        ...
        echo'<p>Votre message a bien été ajouté!<br /><br />Cliquez <a href="../vue/forum/index.php">ici</a> pour revenir à l index du forum<br />
...
<?php
    ...
    if (empty($message))
    {
        echo'<p>Votre message est vide, cliquez <a href="../vue/forum/poster.php?action=repondre&amp;t='.$topic.'">ici</a> pour recommencer</p>';
    }
    else //Sinon, si le message n'est pas vide
    {
        ...
        Cliquez <a href="../vue/forum/index.php">ici</a> pour revenir à l index du forum<br />
...
case "edit": //Si on veut éditer le post
    ...
    $query=$bdd->prepare('SELECT post_createur, post_texte, post_time, topic_id, auth_modo
    FROM forum_post
...
case "delete": //Si on veut supprimer le post
    //On récupère la valeur de p
    $post = (int) $_GET['p'];
    $query=$bdd->prepare('SELECT post_createur, post_texte, forum_id, topic_id, auth_modo
    FROM forum_post
    LEFT JOIN forum_forum ON forum_post.post_forum_id = forum_forum.forum_id
    WHERE post_id=:post');
    $query->bindValue(':post',$post,PDO::PARAM_INT);
    ...
    else //Sinon ça roule et on continue
    {
        ...
        elseif ($data_post['topic_last_post']==$post)  //Si le message est le dernier
        {
            ...
            Cliquez <a href="../vue/forum/index.php">ici</a> pour revenir à l index du forum</p>';
...
case "delete_topic":
    ...
    else //Sinon ça roule et on continue
    {
        ...
        $query=$bdd->prepare('SELECT post_createur, COUNT(*) AS nombre_mess FROM forum_post
        ...
        {
            $query=$bdd->prepare('UPDATE forum_membres
...
case "deplacer":
    ...
    else //Sinon ça roule et on continue
    {
        ...
        //On déplace le topic
        $query=$bdd->prepare('UPDATE forum_topic SET forum_id = :dest WHERE topic_id = :topic');
        ...
        $query=$bdd->prepare('UPDATE forum_post SET post_forum_id = :dest
        WHERE topic_id = :topic');
        $query->bindValue(':dest',$destination,PDO::PARAM_INT);
        $query->bindValue(':topic',$topic,PDO::PARAM_INT);
        $query->execute();
        ...
        $query=$bdd->prepare('SELECT post_id FROM forum_post WHERE post_forum_id = :ori
        ORDER BY post_id DESC LIMIT 0,1');
        ...
        $query=$bdd->prepare('UPDATE forum_forum SET forum_post = forum_post - :nbr, forum_topic = forum_topic - 1,
        ...
        $query=$bdd->prepare('SELECT post_id FROM forum_post WHERE post_forum_id = :dest
        ORDER BY post_id DESC LIMIT 0,1');
        $query->bindValue(':dest',$destination,PDO::PARAM_INT);
        $query->execute();
        $data=$query->fetch();
        ...
        $query=$bdd->prepare('UPDATE forum_forum SET forum_post = forum_post + :nbr,
